...
//       // }
//       // Process the AI response to inject document links
//       const processedContent = processDocumentLinks(
//         result.ai_response || "I couldn't find any relevant documents in our archive.",
//         result.document_metadata || []
//       );

//       // Remove the loading message and add the processed response
//       const filteredMessages = chatMessages.filter(msg => !msg.isLoading);
//       setChatMessages([
//         ...filteredMessages,
//         {
...
//         // For PowerPoint and Word files, show message and download
//         // if (confirm(`Browser cannot display ${fileExtension.toUpperCase()} files directly. Do you want to download the file?`)) {
//         if (window.confirm(`Browser cannot display ${fileExtension.toUpperCase()} files directly. Do you want to download the file?`)) {
//           const response = await axios({
//             url: `${API_URL}/archive/projects/${projectId}/documents/${documentId}/view`,
//             method: 'GET', 
//             responseType: 'blob',
...
// // Function to process AI response and inject document links
// const processDocumentLinks = (aiResponse, documentMetadata) => {
//   if (!aiResponse || !documentMetadata || documentMetadata.length === 0) {
//     return aiResponse;
//   }
...
//     if (!doc.filename || !doc.project_id || !doc.document_id) {
//       return; // Skip if missing required data
//     }

//     // Create regex to find the filename in the text
//     // We'll look for the filename with or without special markdown characters
//     const escapedFilename = doc.filename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    
//     // Look for patterns like:
//     // "filename.ext: reason" or "filename.ext - reason" or just "filename.ext"
//     const patterns = [
//       new RegExp(`(\\* |\\- |\\d+\\. )?${escapedFilename}(?=:)`, 'gi'),  // "filename:" pattern
//       new RegExp(`(\\* |\\- |\\d+\\. )?${escapedFilename}(?= -)`, 'gi'), // "filename -" pattern  
//       new RegExp(`(\\* |\\- |\\d+\\. )?${escapedFilename}(?=\\s|$)`, 'gi') // "filename" at end or before space
//     ];

//     patterns.forEach(pattern => {
//       processedResponse = processedResponse.replace(pattern, (match) => {
//         // Extract any list prefix (*, -, 1., etc.)
//         const listPrefix = match.match(/^(\* |\- |\d+\. )?/)[0] || '';
//         const cleanFilename = match.replace(/^(\* |\- |\d+\. )?/, '');
        
//         // Create markdown link with custom document protocol
//         const markdownLink = `[${cleanFilename}](document:${doc.project_id}/${doc.document_id})`;
        
//         return listPrefix + markdownLink;
//       });
//     });
//   });
